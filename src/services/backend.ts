...
function fromRow(row: any): LessonProgress {
  return { id: row.id, profileId: row.profile_id, lessonId: row.lesson_id, status: row.status, score: row.score ?? 0, mastery: Number(row.mastery ?? 0), attempts: row.attempts ?? 0, startedAt: row.started_at ?? undefined, completedAt: row.completed_at ?? undefined, nextReviewAt: row.next_review_at ?? undefined, xp: row.xp ?? 0, stars: row.stars ?? 0 }
}
export const backend = {
  ...
  async bootstrap(): Promise<AppState | null> {
    ...
    for (const attempt of attempts) {
      ...
      if (id) activityMastery[id] = (activityMastery[id] ?? 0) * .7 + (ok ? .3 : 0)
    }
    const ps = Object.values(progress) as LessonProgress[]
    return { profile, progress, activityMastery, xp: ps.reduce((n, p) => n + p.xp, 0), stars: ps.reduce((n, p) => n + p.stars, 0), lastLessonId: ps.find(p => p.status === 'in_progress')?.lessonId }
  },
  ...
  async event(eventName: string, payload: Record<string, unknown> = {}) {
    try { await call('analytics', { eventName, payload }) } catch { /* analytics must never block gameplay */ }
  }
}
